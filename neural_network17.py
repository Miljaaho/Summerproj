...
# Data is filtered out & NMI_corr included
...
prefix = 'macro_model_with_filter_{}_ep_{}_batch_{}'.format(Fname, N_EPOCHS, B_SIZE)
...
if __name__ == '__main__':
	...
	for mode in ('normal', 'filter'):
		#to create a new folder to save all the data from the 2 different modes 
		PATH=dir + '/' + mode
		os.makedirs(PATH)

		if mode == 'filter':
		        predictions = model.predict(features)
                        mask = np.ones(labels.shape[0]).astype(np.bool)
                        mask[np.logical_and(predictions.flatten() < 0.35, labels.flatten())] = False
		        print 'Dropping', len(mask) - mask.sum(), 'training samples.'
  			features = [f[mask, :] for f in features]
                        labels = labels[mask]
		...
		print 'Saving'
		with open(Fname + '/'+ mode + '/' + prefix + 'model.json', 'w') as outf:
		    outf.write(model.to_json())
		model.save_weights(Fname + '/' + mode + '/' + prefix + '.h5', overwrite=True)
		try:
		    with open(Fname + '/' + mode + '/' + prefix + 'history.cP', 'w') as outf:
			import cPickle
		...
		plt.legend(loc=0)
		plt.savefig(Fname + '/' + mode + '/' + prefix + 'loss_epoch.png')
		plt.semilogx()
		plt.savefig(Fname + '/' + mode + '/' + prefix + 'loss_epoch_log.png')
		plt.close()
		...
		plt.ylabel('Accuracy')
		plt.legend(loc=0)
		plt.savefig(Fname + '/' + mode + '/' + prefix + 'acc_epoch.png')		
		plt.semilogx()
		plt.savefig(Fname + '/' + mode + '/' + prefix + 'acc_epoch_log.png')
		plt.close()
		...
		pos_labels = labels_test == 1
		y_pred = model.predict(features_test, verbose=1)[:, 0]
		...
		plt.savefig(Fname + '/' + mode + '/' + prefix + 'values_hist.png')
		plt.close()
		...
		plt.savefig(Fname + '/' + mode + '/' + prefix + 'values_hist_normed.png')
		plt.close()
		...
		plt.title('ROC curve')
		plt.savefig(Fname + '/'+ mode + '/'  + prefix + 'roc_curve.png')
		plt.close()
		...
		plt.savefig(Fname + '/'+ mode + '/'  + prefix + 'score_fpr.png')
		plt.close()
		...
		plt.ylabel('Recall')
		plt.savefig(Fname + '/'+ mode + '/'  + prefix + 'pr_curve.png')
		plt.close()
